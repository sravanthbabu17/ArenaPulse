...
const aiLimiter = rateLimit({
  ...
  message: {
    error:
      "AI generation limit reached (10/min). Please wait before generating another report.",
  },
});
...
if (process.env.NODE_ENV !== "test") {
  app.listen(config.PORT, () => {
    console.log(
      `🏟️ ArenaPulse backend listening on port ${config.PORT} [Mode: ${config.GEMINI_API_KEY ? "LIVE" : "SIMULATION"}]`,
    );
  });
}
