  ...
  incidents: [
    {
      ...
      description: "Ticketing turnstile failure causing line lengths to exceed 18 minutes."
    },
    {
      id: "sec-102",
      ...
      severity: "warning",
      timestamp: new Date().toLocaleTimeString(),
      description: "Spectator reports mild heat exhaustion. First responders dispatched."
    }
  ],
  zones: [
    { id: "zone-north", name: "North Zone (Blue)", occupancy: 68, status: "comfortable" },
    { id: "zone-east", name: "East Zone (Amber)", occupancy: 82, status: "busy" },
    { id: "zone-south", name: "South Zone (Green)", occupancy: 58, status: "comfortable" },
    { id: "zone-west", name: "West Zone (Red)", occupancy: 93, status: "critical" }
  ]
};
...
function fluctuateState(): void {
  ...
  state.avgGateWait = Math.max(3, Math.min(25, state.avgGateWait + (Math.random() - 0.5) * 0.8));

  // Fluctuate seating quadrant percentages
  state.zones.forEach(z => {
    z.occupancy = Math.max(20, Math.min(100, z.occupancy + Math.floor((Math.random() - 0.5) * 4)));
...
operationsRouter.post('/resolve', (req: Request, res: Response): void => {
  ...
  if (!incidentId || typeof incidentId !== 'string') {
    res.status(400).json({ error: "Missing or invalid incidentId in request body." });
    return;
  }
  ...
  if (state.incidents.length < initialLen) {
    res.json({ success: true, message: `Incident ${incidentId} resolved successfully.`, incidents: state.incidents });
  } else {
...
operationsRouter.post('/carbon', (req: Request, res: Response): void => {
  ...
  if (amount === undefined || typeof amount !== 'number') {
    res.status(400).json({ error: "Missing or invalid amount in request body. Must be a number." });
    return;
  }
  ...
  if (amount > 1000) {
    res.status(400).json({ error: "Amount exceeds maximum allowed single-submission cap of 1000 kg." });
    return;
  }
 ...
 */
operationsRouter.post('/briefing', async (_req: Request, res: Response): Promise<void> => {
  try {
    fluctuateState();
...
${stateContext}`;
        ...
        briefingText = await callGemini("Generate Operations Briefing Report", systemPrompt);
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : 'Unknown error';
        console.warn("Briefing generation failed, using simulation:", message);
        briefingText = `⚠️ **Gemini API Error (Simulation Fallback):** ${message}\n\n` + runSimulatedBriefing();
      }
    } else {
 ...
 */
operationsRouter.post('/sop', async (req: Request, res: Response): Promise<void> => {
  try {
    const { incidentId, customContext } = req.body as { incidentId?: string; customContext?: string };
    if (!incidentId || typeof incidentId !== 'string') {
      res.status(400).json({ error: "Missing or invalid incidentId in request body." });
      return;
    }
...
Ensure the layout is formatted in Markdown with header '### SOP: [Incident Name]' followed by direct, actionable 'Operational Directives'.`;
        ...
        sopText = `⚠️ **Gemini API Error (Simulation Fallback):** ${message}\n\n` + runSimulatedSOP(incidentId, customContext);
      }
    } else {
...
function runSimulatedBriefing(): string {
  const criticalZones = state.zones.filter(z => z.status === 'critical').map(z => z.name).join(', ');
  const activeAlerts = state.incidents.map(inc => `* **${inc.name}** (${inc.severity.toUpperCase()}): ${inc.description}`).join('\n');
