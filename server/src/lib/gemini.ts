 ...
 */
export async function callGemini(promptText: string, systemInstruction: string): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Gemini API request failed with code ${response.status}: ${errorBody}`);
  }
  ...
  if (!resultText) {
    throw new Error('Invalid or empty candidates structure in Gemini API response.');
  }
